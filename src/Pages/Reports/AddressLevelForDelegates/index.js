    ...
    try {
      const tableHtml = `
        <table>
          ...
          <tbody>
            ${data
              .map((item, index) => `<tr><td>${index + 1}</td><td>${item}</td></tr>`)
              .join('')}
          </tbody>
        </table>
      `;
      ...
      <div className={`yourComponent ${isTableExpanded ? "expanded" : ""}`}>
        ...
        {isTableExpanded && (
          <div className="container mx-auto mt-5">
            <button onClick={handleExportToWord} className="btn" style={{margin: "15px"}}>Export to Word</button>
            <div className="overflow-x-auto">
              <table className="table-auto w-full" id="table-to-export" style={wordTable}>
                <thead>
                  <tr>
                    {/* <th>Serial Number</th>
          <th>Data</th>
