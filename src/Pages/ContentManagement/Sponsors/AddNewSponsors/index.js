...
const AddNewSponsors = () => {
  ...
  const addRow = () => {
    // Only add a new row when "Add" button is clicked
  ...
  const saveRow = (id, newData) => {
    ...
    setIsNewRowAdding(false); // Exit new row adding mode after saving
  };
                      ...
                      {row.cells.map((cell) => {
                        return (
                          ...
                          >
                            {cell.column.id === "id" ? (
                                cell.render("Cell")
                              ) : isNewRowAdding && cell.column.id !== "id" ? (
                                // Render input fields only for the new row
                                <input
                                  type="text"
                                  value={row.values[cell.column.id]}
                                  onChange={(e) =>
                                    saveRow(row.original.id, {
                                      [cell.column.id]: e.target.value,
                                    })
                                  }
                                />
                              ) : (
                                // Render cell data for existing rows
                                cell.render("Cell")
                              )}
                          </td>
                        );
